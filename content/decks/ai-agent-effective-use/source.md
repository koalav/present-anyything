...
> AI 에이전트의 효과적인 활용
> 모바일 보안 감사에서 실행 · 검증 루프로

배경지식 / 발표 멘트:

오늘의 핵심은 "AI를 더 잘 질문하는 법"이 아니라, "AI가 Android 앱 컨텍스트와 분석 도구를 사용해 모바일 보안 감사 작업을 수행하게 만들고, 그 결과를 검증 가능하게 만드는 법"입니다.
...
> AI Agent
> = Model + Android App Context + Tools + Verification

배경지식 / 발표 멘트:

에이전트는 단순 챗봇이 아닙니다. 모델이 있고, 외부 도구가 있고, 작업에 필요한 Android 앱 컨텍스트가 있고, 마지막으로 결과를 확인하는 검증 루프가 있어야 합니다. OpenAI의 도구 문서도 모델이 도구를 자동 선택할 수 있지만, 도구 접근은 `tools` 설정과 애플리케이션 실행 흐름으로 연결된다고 설명합니다. [R2]
...
> 질문 + 도구 = 작업 루프
> Android Docs / APK / Python / Device

배경지식 / 발표 멘트:

최근의 AI 사용 환경은 단순 텍스트 생성에서 벗어났습니다. 모바일 보안 감사에서는 Android 문서, APK와 소스, 구조화 추출기, ADB/Frida 같은 실행 도구, MCP 서버가 붙습니다. Reasoning은 외부 도구라기보다 모델이 문제를 푸는 방식에 가깝기 때문에 도구 목록과는 구분해서 설명하는 편이 정확합니다. [R2][R7]
...
기존 ChatGPT식 감사 작업은 사람이 오케스트레이터였습니다. AI가 점검 항목을 제안하면 사람이 Manifest를 찾고, jadx나 adb를 실행하고, 결과를 다시 요약해 모델에 붙여넣어야 했습니다.
...
감사 Harness는 APK/source를 로드하고, Manifest와 decompiled code에서 facts를 추출하고, AI에게 위험 판단을 맡기고, 부족한 증거가 있으면 adb나 Frida를 다시 호출합니다. 중요한 차이는 사람이 복사하는 것이 아니라 에이전트가 분석 루프를 직접 돈다는 점입니다. [R4]
...
필수 요소는 여섯 가지입니다. 첫째, APK, source, build variant를 구조화하는 Target Intake. 둘째, Manifest, Gradle, API 정책만 넣는 Android Context. 셋째, jadx, adb, Frida 입력과 권한, 로그를 고정하는 Tool Contract. 넷째, 결정적 facts를 JSON으로 만드는 Extractor. 다섯째, 언제 끝내고 언제 물어볼지 정하는 Stop Conditions. 여섯째, 실행 증거를 남기는 Evidence Report입니다.

필요 그림: 6개 블록 체크리스트.

## 19-6. 결정적인 출력은 코드가 만듭니다
...
## 23-26. 빠른 AI는 검증으로 묶습니다

화면 텍스트:

> Behavior: 빠르고 많이 바꾸므로 범위와 리뷰가 필요합니다.
> Command: 분석 명령이 실행 가능한 성공 기준입니다.
> Facts: 먼저 구조화된 facts가 목표를 고정합니다.
> Design: Extractor와 AI 판단을 분리합니다.

배경지식 / 발표 멘트:

AI는 매우 빠르게 작업하지만, 항상 우리가 원하는 방향으로만 움직이지는 않습니다. 그래서 자연어 목표를 manifest extractor, aapt, adb, Frida 같은 실행 가능한 분석 명령으로 바꿔야 합니다. 먼저 구조화된 facts를 만들면 에이전트가 판단해야 할 대상이 명확해지고, extractor와 AI 판단을 분리하면 누락과 비용을 줄일 수 있습니다. [R7]
...
AI에게 "알아서 해"라고 하면 모델의 일반 지식에 의존합니다. 모바일 보안 감사에서는 AndroidManifest, Gradle build context, minSdk/targetSdk, flavor, 사내 exported component 정책, 허용된 deep link 예외를 컨텍스트로 제공해야 합니다. MCP Resources나 파일 검색은 이런 컨텍스트를 공급하는 방식이 될 수 있습니다. [R1][R2]
...
처음부터 AI에게 모든 파일을 읽히지 말고 먼저 대상 APK/source와 build variant를 고정합니다. 그 다음 parser와 도구로 facts를 추출하고, AI에게 위험 판단을 맡긴 뒤, 재현 가능한 증거를 리뷰합니다. Codex와 Claude Code 모두 승인 모드, 권한 설정, 도구 제한을 통해 이 흐름을 통제할 수 있습니다. [R3][R4]
...
AI 에이전트 활용의 핵심은 도구를 부정하는 것이 아닙니다. 도구는 실행력을 만들고, 검증은 신뢰를 만듭니다. 모바일 보안 감사에서는 올바른 Android context로 시작하고, 제한된 device/tool 권한 안에서 실행하고, 재현 가능한 evidence로 끝나야 합니다.

필요 그림: Android context + Tools + Evidence + Human Review 최종 요약 이미지
